...
async function updateAvis(id, data) {
  const res = await fetch(`${SUPABASE_URL}/avis?id=eq.${id}`, {
    method: "PATCH",
...
function PlatformBadge({ platform }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: p.bg, color: p.color, border: `1px solid ${p.color}30`, borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>
      <span style={{ fontWeight: 900, fontSize: 13 }}>{p.icon}</span>{p.name}
    </span>
  );
}
...
export default function App({ user, onLogout }) {
  ...
  const generateAIReply = async (review) => {
    ...
    setReplyDraft("");
    try {
      const prompt = `Tu es gérant d'un restaurant. Réponds en français à cet avis (${review.rating}/5★) de ${review.author}: "${review.text}". Réponds de façon personnalisée, 2-3 phrases, professionnel et humain. Ne commence pas par Cher/Chère.`;
      ...
      const data = await res.json();
      setReplyDraft(data.text || "Erreur de génération.");
    } catch {
  ...
  const publishReply = async (reviewId) => {
    ...
    try {
      ...
      setReplyDraft("");
      showNotif("Réponse publiée avec succès! ✓");
    } catch (e) {
  ...
  return (
    <div style={{ fontFamily: "'Syne', sans-serif", background: "#0A0A0F", minHeight: "100vh", width: "100vw", color: "#E8E8F0" }}>
      <style>{`
        ...
        <div style={{ padding: "28px 16px", maxWidth: "100vw", margin: "0 auto" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {r.replied && r.reply && (
                          <div style={{ marginTop: 12, padding: "12px 14px", background: "#0D1A0D", borderRadius: 10, borderLeft: "3px solid #10B981" }}>
                            <div style={{ fontSize: 11, color: "#10B981", fontWeight: 700, marginBottom: 4 }}>✓ Réponse publiée</div>
          ...
          {activeTab === "répondre" && (
            <div style={{ animation: "fadeUp 0.4s ease" }}>
              ...
              <p style={{ color: "#555", marginBottom: 24, fontSize: 14 }}>{pendingReviews.length} avis en attente</p>
                  ...
                  <div style={{ ...card, position: "sticky", top: 80, height: "fit-content" }}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                      ...
                      <button onClick={() => publishReply(selectedReview.id)} disabled={!replyDraft.trim() || publishing} style={{ flex: 2, padding: "11px", background: replyDraft.trim()?"linear-gradient(135deg, #6366F1, #8B5CF6)":"#1A1A2E", border: "none", borderRadius: 10, color: replyDraft.trim()?"#fff":"#444", fontWeight: 700, cursor: replyDraft.trim()?"pointer":"not-allowed", fontSize: 14, fontFamily: "Syne", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                        {publishing ? <><span style={{ animation: "pulse 1s infinite" }}>●</span> Publication...</> : "🚀 Publier la réponse"}
                      </button>
                    </div>
                  </div>
                )}
